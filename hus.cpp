...
uint64_t private_m::AllocMdlMemory(HANDLE iqvw64e_device_handle, uint64_t size, uint64_t* mdlPtr) {
	/*added by psec*/
...
uint64_t private_m::AllocIndependentPages(HANDLE device_handle, uint32_t size)
{
...
uint64_t private_m::MapDriver(HANDLE iqvw64e_device_handle, BYTE* data, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, AllocationMode mode, bool PassAllocationAddressAsFirstParam, mapCallback callback, NTSTATUS* exitCode) {
...
void private_m::RelocateImageByDelta(portable_executable::vec_relocs relocs, const uint64_t delta) {
...
bool private_m::FixSecurityCookie(void* local_image, uint64_t kernel_image_base)
{
...
bool private_m::ResolveImports(HANDLE iqvw64e_device_handle, portable_executable::vec_imports imports) {
	for (const auto& current_import : imports) {
		ULONG64 Module = utils2::GetKernelModuleAddress(current_import.module_name);
		if (!Module) {
