...
std::wstring utils2::GetFullTempPath() {
...
bool utils2::ReadFileToMemory(const std::wstring& file_path, std::vector<uint8_t>* out_buffer) {
...
bool utils2::CreateFileFromMemory(const std::wstring& desired_file_path, const char* address, size_t size) {
...
uint64_t utils2::GetKernelModuleAddress(const std::string& module_name) {
	void* buffer = nullptr;
	DWORD buffer_size = 0;
...
BOOLEAN utils2::bDataCompare(const BYTE* pData, const BYTE* bMask, const char* szMask) {
...
uintptr_t utils2::FindPattern(uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, const char* szMask) {
...
PVOID utils2::FindSection(const char* sectionName, uintptr_t modulePtr, PULONG size) {
