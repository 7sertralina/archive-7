...
bool service::RegisterAndStart(const std::wstring& driver_path) {
	...
	if (Status == 0xC0000603) {
		Log("[-] Your vulnerable driver list is enabled and have blocked the driver loading, you must disable vulnerable driver list to use private_m with intel driver" << std::endl);
