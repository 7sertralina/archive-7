...
std::wstring intel_driver::GetDriverPath() {
	std::wstring temp = utils2::GetFullTempPath();
	if (temp.empty()) {
		return L"";
	}
...
bool intel_driver::IsRunning() {
	const HANDLE file_handle = CreateFileW(L"\\\\.\\Parallel0", FILE_ANY_ACCESS, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
...
HANDLE intel_driver::Load(bool clean) {
	...
	if (!utils2::CreateFileFromMemory(driver_path, reinterpret_cast<const char*>(rtcore64_driver_resource::driver), sizeof(rtcore64_driver_resource::driver))) {
	...
	ntoskrnlAddr = utils2::GetKernelModuleAddress("ntoskrnl.exe");
	if (ntoskrnlAddr == 0) {
	...
	uintptr_t win32k = utils2::GetKernelModuleAddress("win32k.sys");
	if (!win32k)
	{
...
bool intel_driver::ClearWdFilterDriverList(HANDLE device_handle) {
	...
	auto WdFilter = utils2::GetKernelModuleAddress("WdFilter.sys");
	if (!WdFilter) {
...
uintptr_t intel_driver::FindPatternAtKernel(HANDLE device_handle, uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, const char* szMask) {
	...
	auto result = utils2::FindPattern((uintptr_t)sectionData.get(), dwLen, bMask, szMask);
...
uintptr_t intel_driver::FindSectionAtKernel(HANDLE device_handle, const char* sectionName, uintptr_t modulePtr, PULONG size) {
	...
	ULONG sectionSize = 0;
	uintptr_t section = (uintptr_t)utils2::FindSection(sectionName, (uintptr_t)headers, &sectionSize);
...
bool intel_driver::ClearKernelHashBucketList(HANDLE device_handle) {
	uint64_t ci = utils2::GetKernelModuleAddress("ci.dll");
	if (!ci) {
